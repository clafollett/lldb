...
And, in `test/fleet-posture.test.ts`, that the variable lands on a container that will actually act
on it: every container receiving `LLDB_REQUIRE_FLEET_TOKEN` must also receive `LLDB_FLEET_TOKEN`
(asserting a closed fleet while supplying no secret is a process that refuses to start), and its
`Command` must name a binary that calls `check_fleet_posture_from_env` — otherwise the deployment's
claim is never evaluated and the evidence of that is nothing at all.

The set of checking binaries is **read, not restated**: [`fleet-posture.json`](fleet-posture.json)
is generated by `scripts/check-fleet-posture.sh --json` from `cargo metadata`, and that script's
normal run — CI's `Fleet posture` step, which has the cargo this job does not — fails when the
committed copy is stale. Do not hand-edit it. Two consequences worth knowing: a container whose
binary is **absent** from the roster fails rather than being skipped as unknown, which is what makes
a new binary break the build by default; and a container whose `Command` cannot be mapped to a
binary at all — no `Command`, an unresolved `Ref`, a `sh -c` wrapper — fails for the same reason,
since a guard that passes what it cannot read is the defect it exists to catch.
